...
import { submitRentRequestForm } from "@/app/lib/hooks/Request.hook";
...
type BuySecondSlideProps = {
  ...
  rentRequestFirstPageFormData: TUserPage;
};

const BuySecondSlide: React.FC<BuySecondSlideProps> = ({
  handlePrevClick,
  rentRequestFirstPageFormData,
}) => {
  ...
  const rentMutation = useMutation<void, Error, TCreateBuyRequestPayload>({
    mutationFn: (credentials: TCreateBuyRequestPayload) =>
      submitRentRequestForm(credentials),
    onError: (error) => {
  ...
  const handleBuyRequestSubmit = async (
    ...
    if (userBuy.firstName === "") {
      ...
      const payload: TCreateBuyRequestPayload = {
        locationId: rentRequestFirstPageFormData.locationId,
        otherLocationIds: rentRequestFirstPageFormData.otherLocationId
          ? [rentRequestFirstPageFormData.otherLocationId]
          : [],
        propertyPreferenceId: rentRequestFirstPageFormData.propertyId,
        propertySubPreferenceId: rentRequestFirstPageFormData.subPropertyId,
        bedrooms: rentRequestFirstPageFormData.selectedroomNumber,
        budget: rentRequestFirstPageFormData.selectedBudgetRange,
        otherDetail: rentRequestFirstPageFormData.specificationDesc,
      ...
      await rentMutation.mutateAsync(payload);
    }
  };
